...
const Signup = () => {
  ...
  const handleSignup = (e) => {
    ...
    let hasError = false;
    if (!firstName) {
      setFirstNameError(language === 'ar' ? 'يرجى إدخال الاسم الأول' : 'Please enter your first name');
      hasError = true;
    } else {
      setFirstNameError('');
    }
    if (!lastName) {
      setLastNameError(language === 'ar' ? 'يرجى إدخال الاسم الثاني' : 'Please enter your last name');
      hasError = true;
    } else {
      setLastNameError('');
    }
    ...
    if (!confirmPassword) {
      setConfirmPasswordError(language === 'ar' ? 'يرجى تأكيد كلمة المرور' : 'Please confirm your password');
      hasError = true;
    } else if (password !== confirmPassword) {
      setConfirmPasswordError(language === 'ar' ? 'كلمة المرور غير متطابقة' : 'Passwords do not match');
      hasError = true;
    } else {
  ...
  return (
    <Container>
      <LoginCard>
        ...
        <LoginSubtitle>
          {language === 'ar' ? 'انضم إلينا اليوم وابدأ رحلتك' : 'Join us today and start your journey'}
        </LoginSubtitle>
        <form onSubmit={handleSignup}>
          <NameRow>
            <FormGroup style={{ flex: 1 }}>
              ...
              />
              {firstNameError && (
                <div style={{ color: 'red', fontSize: '13px', marginTop: '4px', textAlign: 'end' }}>{firstNameError}</div>
              )}
            </FormGroup>
            <FormGroup style={{ flex: 1 }}>
              ...
              <input
                ...
                placeholder={language === 'ar' ? 'أدخل اسمك الثاني' : 'Enter your last name'}
                style={lastNameError ? { borderColor: 'red' } : {}}
              />
              {lastNameError && (
                <div style={{ color: 'red', fontSize: '13px', marginTop: '4px', textAlign: 'end' }}>{lastNameError}</div>
              )}
            </FormGroup>
          </NameRow>
          <FormGroup>
            ...
            />
            {phoneError && (
              <div style={{ color: 'red', fontSize: '13px', marginTop: '4px', textAlign: 'end' }}>{phoneError}</div>
            )}
          </FormGroup>
          <FormGroup>
          ...
          <FormGroup>
            ...
            {confirmPasswordError && (
              <div style={{ color: 'red', fontSize: '13px', marginTop: '4px', textAlign: 'end' }}>{confirmPasswordError}</div>
            )}
          </FormGroup>
          <FormOptions>
            <RememberMe>
        ...
        <SocialLoginSection>
          <Divider>
            <Line />
            <DividerText>{language === 'ar' ? 'أو سجل باستخدام' : 'Or sign up with'}</DividerText>
            <Line />
          </Divider>
