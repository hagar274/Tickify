...
const Flights = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    console.log('تم الضغط على ابحث', {origin, destination, departureDate, returnDate, cabinClass});
    if (validateForm()) {
  ...
  return (
    ...
    <WelcomeContainer>
        ...
        <SearchBox>
          <SearchForm onSubmit={handleSubmit}>
            <OriginDestinationGroup>
              <FormGroup>
                <label htmlFor="origin">{language === 'ar' ? 'من' : 'From'}</label>
                <input 
                  type="text" 
                  id="origin" 
              ...
              <FormGroup>
                <label htmlFor="destination">{language === 'ar' ? 'إلى' : 'To'}</label>
                <input 
                  type="text" 
                  id="destination" 
            ...
            <FormGroup>
              <label htmlFor="departure-date">{language === 'ar' ? 'تاريخ المغادرة' : 'Departure Date'}</label>
              <input 
                type="date" 
                id="departure-date"
                value={departureDate}
            ...
            <FormGroup>
              <label htmlFor="return-date">{language === 'ar' ? 'تاريخ العودة' : 'Return Date'}</label>
              <input 
                type="date" 
                id="return-date"
                value={returnDate}
            ...
            <FormGroup>
              <label htmlFor="cabin-class">{language === 'ar' ? '' : 'Cabin Class'}</label>
              <select 
                id="cabin-class"
                value={cabinClass}
                onChange={(e) => setCabinClass(e.target.value)}
              >
                <option value="economy">{language === 'ar' ? 'Economy' : 'Economy'}</option>
                  <option value="business">{language === 'ar' ? 'Business' : 'Business'}</option>
                  <option value="economy">{language === 'ar' ? 'FirstClass' : 'FirstClass'}</option>
                <option value="vip">{language === 'ar' ? 'VIP' : 'VIP'}</option>
              </select>
            </FormGroup>
            <SearchButton type="submit">{language === 'ar' ? 'ابحث' : 'Search'}</SearchButton>
          </SearchForm>
        </SearchBox>
      </HeroSection>
      ...
      <FeaturesSection>
        ...
        <p>{language === 'ar' ? 'اكتشف أكثر من 1000 وجهة حول العالم' : 'Discover over 1,000 destinations around the world'}</p>
        </FeatureCard>
          ...
          <p>{language === 'ar' ? 'نضمن لك أمان معاملاتك وحماية بياناتك' : 'We guarantee the security of your transactions and the protection of your data.'}</p>
          </FeatureCard>
      ...
      <OffersSection>
        ...
        <OffersGrid>
          <OfferCard>
            ...
            <OfferContent>
              <h3>{language === 'ar' ? 'عرض دبي المميز' : 'Dubai Special Offer'}</h3>
              <p className="offer-price">{language === 'ar' ?  'يبدأ من 1999 ريال' : 'Starting from 1999 SAR'}</p>
              <p>{language === 'ar' ? '3 ليالي في فندق 5 نجوم مع تذاكر الطيران' : '3 nights in a 5-star hotel with flight tickets'}</p>
            </OfferContent>
          </OfferCard>
          ...
          <OfferCard>
            ...
            <OfferContent>
              ...
              <p className="offer-price">{language === 'ar' ? 'يبدأ من 2499 ريال' : 'Starting from 2499 SAR'}</p>
              <p>{language === 'ar' ?  '5 ليالي شاملة الفندق والطيران' : '5 nights including hotel and flight'}</p>
            </OfferContent>
          </OfferCard>
          ...
          <OfferCard>
            ...
            <OfferContent>
              ...
              <p className="offer-price">{language === 'ar' ? 'يبدأ من 4999 ريال' : 'Starting from 4999 riyals'}</p>
              <p>{language === 'ar' ? '7 ليالي في منتجع فاخر مع الطيران' : '7 nights in a luxury resort with flights'}</p>
          </OfferContent>
          </OfferCard>
          ...
          <OfferCard>
            ...
            <OfferContent>
              ...
              <p className="offer-price">{language === 'ar' ? 'يبدأ من 3999 ريال' : 'Starting from 3999 riyals'}</p>
              <p>{language === 'ar' ? '4 ليالي في فندق 5 نجوم مع تذاكر الطيران' : '4 nights in a 5-star hotel with flight tickets'}</p>
            </OfferContent>
          </OfferCard>
        </OffersGrid>
      </OffersSection>
